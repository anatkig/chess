...
const Piece = ({
  ...
  rowIndex: number;
  cellIndex: number;
  writeOnStartCoordinates: (dragPosition: [number, number], type: string, color: string) => void;
}) => {
  return (
    <div
      className="flex place-content-center cursor-pointer rotate-180 piece"
      draggable
      onDragStart={() => writeOnStartCoordinates([rowIndex, cellIndex], type, color)}
    >
      {type === "knight" ? (
        <Knight color={color} />
      ) : type === "queen" ? (
...
const mapDispatchtoProps = (dispatch: Dispatch) => {
  return {
    writeOnStartCoordinates: (dragPosition: [number, number], type: string, color: string) =>
